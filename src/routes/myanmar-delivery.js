...
const myanmarDeliveryService = createNotionService(DATABASE_IDS.myanmarDeliveryData);
...
// POST /api/myanmar-delivery/list - 전체 조회
router.post('/list', async (req, res) => {
  try {
    ...
    const result = await myanmarDeliveryService.getAllItems(filterObj, sortsArr);
    ...
    logger.error('미얀마배송데이터 조회 오류:', error);
    res.status(500).json(error);
  }
});

// POST /api/myanmar-delivery/get - 특정 항목 조회
router.post('/get', async (req, res) => {
  try {
    ...
    const result = await myanmarDeliveryService.getItem(pageId);
    ...
    logger.error('미얀마배송데이터 항목 조회 오류:', error);
    res.status(500).json(error);
  }
});

// POST /api/myanmar-delivery - 항목 추가
router.post('/', async (req, res) => {
  try {
    ...
    const result = await myanmarDeliveryService.addItem(properties);
    res.status(201).json(result);
  } catch (error) {
    logger.error('미얀마배송데이터 항목 추가 오류:', error);
    res.status(500).json(error);
  }
});

// PATCH /api/myanmar-delivery/:pageId - 항목 업데이트
router.patch('/:pageId', async (req, res) => {
  try {
    ...
    const result = await myanmarDeliveryService.updateItem(pageId, properties);
    res.json(result);
  } catch (error) {
    logger.error('미얀마배송데이터 항목 업데이트 오류:', error);
    res.status(500).json(error);
  }
});

// DELETE /api/myanmar-delivery/:pageId - 항목 삭제(아카이브)
router.delete('/:pageId', async (req, res) => {
  try {
    const { pageId } = req.params;
    const result = await myanmarDeliveryService.deleteItem(pageId);
    res.json(result);
  } catch (error) {
    logger.error('미얀마배송데이터 항목 삭제 오류:', error);
    res.status(500).json(error);
  }
});
...
// GET /api/myanmar-delivery/:pageId/properties - 특정 항목의 모든 properties 조회
...
// GET /api/myanmar-delivery/:pageId/properties/:propertyName - 특정 property 조회
...
// PATCH /api/myanmar-delivery/:pageId/properties - 특정 properties 업데이트
router.patch('/:pageId/properties', async (req, res) => {
  try {
    ...
    if (!updates || typeof updates !== 'object') {
      return res.status(400).json({
        success: false,
        error: 'updates 객체가 필요합니다. 예: { "미얀마요청배송비M": { "type": "number", "value": 3500 } }'
      });
    }
...
// PATCH /api/myanmar-delivery/:pageId/properties/:propertyName - 단일 property 업데이트
router.patch('/:pageId/properties/:propertyName', async (req, res) => {
  try {
    ...
    const result = await myanmarDeliveryService.updateItem(pageId, {
