...
// GET /api/exchange/:pageId/properties - 특정 항목의 모든 properties 조회
...
// GET /api/exchange/:pageId/properties/:propertyName - 특정 property 조회
...
// PATCH /api/exchange/:pageId/properties - 특정 properties 업데이트
router.patch('/:pageId/properties', async (req, res) => {
  try {
    ...
    if (!updates || typeof updates !== 'object') {
      return res.status(400).json({
        success: false,
        error: 'updates 객체가 필요합니다. 예: { "마켓환율": { "type": "number", "value": 3.14 } }'
      });
    }
...
// PATCH /api/exchange/:pageId/properties/:propertyName - 단일 property 업데이트
router.patch('/:pageId/properties/:propertyName', async (req, res) => {
  try {
    ...
    const result = await exchangeService.updateItem(pageId, {
