...
// GET /api/warehouses/:pageId/properties - 특정 항목의 모든 properties 조회
...
// GET /api/warehouses/:pageId/properties/:propertyName - 특정 property 조회
...
// PATCH /api/warehouses/:pageId/properties - 특정 properties 업데이트
router.patch('/:pageId/properties', async (req, res) => {
  try {
    ...
    if (!updates || typeof updates !== 'object') {
      return res.status(400).json({
        success: false,
        error: 'updates 객체가 필요합니다. 예: { "배송처명": { "type": "rich_text", "value": "경아로지스틱스" } }'
      });
    }
...
// PATCH /api/warehouses/:pageId/properties/:propertyName - 단일 property 업데이트
router.patch('/:pageId/properties/:propertyName', async (req, res) => {
  try {
    ...
    const result = await warehousesService.updateItem(pageId, {
